...
export default class NeuralNet {
    ...
    constructor(num_in_neurons: number, num_hidden_layers: number, num_out_neurons: number) {
        if (DBG) dbg(`ctor:+ in_neurons=${num_in_neurons} hidden_layers=${num_hidden_layers} out_neurons=${num_out_neurons}`);
    ...
    create_layer(layer_index: number, num_neurons: number) {
        if (DBG) dbg(`create_layer:+ layer_index=${layer_index} num_neurons=${num_neurons}`);
    ...
    add_hidden(num_neurons: number) {
        if (DBG) dbg(`add_hidden:+ num_neurons=${num_neurons}`);
    ...
    start() {
        ...
        for (let l = 0; l < this.max_layers; l++) {
            ...
            if (DBG) dbg(`start: this.layers[${l}].length=${this.layers[l].length} in_layer=${l}`);
    ...
    get_points(): number {
        if (DBG) dbg(`get_points:+- points=${this.points}`);
        return this.points;
    }

    set_inputs(input: number[]) {
        if (DBG) dbg(`set_inputs:+ count=${input.length} input_layer count=${this.layers[0].length}`);
        for (let n = 0; n < this.layers[0].length; n++) {
            // Set then input neuron output
            let neuron = this.layers[0][n];
            neuron.output = input[n];
            if (DBG) dbg(`set_inputs: neuron=${n} output=${neuron.output}`);
        }
        if (DBG) dbg("set_inputs:-");
    }
    ...
    process() {
        ...
        for (let l = 1; l <= this.out_layer; l++) {
            ...
            for (let n = 0; n < layer.length; n++) {
                ...
                if (DBG) dbg(`process: neuron=${n} output=${neuron.output} weighted_sum=${weighted_sum}`);
            }
        }
    ...
    get_outputs(output: number[]) {
        if (DBG) dbg(`get_outputs:+ output.length=${output.length}`);
        ...
        for (let i = 0; i < output.length; i++) {
            ...
            if (DBG) dbg(`get_outputs: output[${i}]=${output[i]}`);
        }
    ...
    adjust_weights(output: number[], target: number[]): number {
        if (DBG) dbg(`adjust_weights:+ output.length=${output.length} target.length=${target.length}`);
        ...
        for (let n = 0; n < output.length; n++) {
            ...
            if (DBG) dbg(`adjust_weights: ${this.out_layer}:${n} err:${err} = target:${target[n]} + output:${output[n]}`);
            ...
            if (DBG) dbg(`adjust_weights: ${this.out_layer}:${n} pd_err:${pd_err} = err:${err} * output[${n}]:${output[n]} * (1.0 - output[${n}]:${output[n]}`);

            // Compute the sum of the square of the error and add to total_error
            let sse = 0.5 * err * err;
            if (DBG) dbg(`adjust_weights: ${this.out_layer}:${n} sse:${sse} = 0.5 * err:${err} * err:${err}`);
            let tmp = this.error;
            this.error = tmp + sse;
            if (DBG) dbg(`adjust_weights: ${this.out_layer}:${n} this.error:${this.error} = this.error:${tmp} + sse:${sse}`);
        }
        if (DBG) dbg(`adjust_weights: out_layer:${this.out_layer} this.error=${this.error}`);
        ...
        for (let l = this.out_layer; l > first_hidden_layer; l--) {
            ...
            if (DBG) dbg(`adjust_weights: cur_layer=${l} prev_layer=${l-1}`);
            ...
            for (let npl = 0; npl < prev_layer.length; npl++) {
                ...
                for (let ncl = 0; ncl < cur_layer.length; ncl++) {
                    ...
                    if (DBG) dbg(`adjust_weights: ${l}:${ncl} pd_err=${pd_err}`);
                    let weight = cur_layer[ncl].weights[npl+1];
                    if (DBG) dbg(`adjust_weights: ${l}:${ncl} weight:${weight} = cur_layer[${ncl}].weights[${npl+1}]`);
                    let tmp = sum_weighted_pd_err;
                    sum_weighted_pd_err = tmp + (pd_err * weight);
                    if (DBG) dbg(`adjust_weights: ${l}:${ncl} sum_weighted_pd_err:${sum_weighted_pd_err} = ${tmp} + (pd_err:${pd_err} * weight:${weight})`);
                }

                let prev_out = prev_layer[npl].output;
                let pd_prev_out = prev_out * (1.0 - prev_out);
                if (DBG) dbg(`adjust_weights: prev_layer:${l-1}:${npl} pd_prev_out:${pd_prev_out} = prev_out:${prev_out} * (1.0 - prev_out:${prev_out})`);
                prev_layer[npl].pd_error = sum_weighted_pd_err * pd_prev_out;
                if (DBG) dbg(`adjust_weights: prev_layer:${l-1}:${npl} pd_error:${prev_layer[npl].pd_error} = sum_weighted_pd_err:${sum_weighted_pd_err} * pd_prev_out:${pd_prev_out}`);
            }
        }
        ...
        for (let l = 1; l <= this.out_layer; l++) {
            let layer = this.layers[l];
            if (DBG) dbg(`adjust_weights: layers:${l} looping`);
            for (let n = 0; n < layer.length; n++) {
                let neuron = layer[n];
                let inputs = neuron.inputs;
                ...
                if (DBG) dbg(`adjust_weights: ${l}:${n} momentum:${momentum} = this.momentum_factor:${this.momentum_factor} * momentums[0]:${momentums[0]}`);
                momentums[0] = (this.learning_rate * pd_err) + momentum;
                if (DBG) dbg(`adjust_weights: ${l}:${n} momentums[0]:${momentums[0]} = (eta:${this.learning_rate} * pd_err:${pd_err}) + momentum:${momentum} bias`);

                let w = weights[0];
                weights[0] = weights[0] + momentums[0];
                if (DBG) dbg(`adjust_weights: ${l}:${n} weights[0]=${weights[0]} = weights[0]:${weights[0]} + momentums[0]:${momentums[0]} bias`);

                // Loop through this neurons input neurons adjusting the weights and momentums
                if (DBG) dbg(`adjust_weights: ${l}:${n} update weights pd_err=${pd_err}`);
                for (let i = 1; i <= neuron.inputs.length; i++) {
                    ...
                    if (DBG) dbg(`adjust_weights: ${l}:${n} momentums[${i}]:${momentums[i]} = (eta:${this.learning_rate} * input:${input} pd_err:${pd_err}) + momentum:${momentum}`);

                    w = weights[i];
                    weights[i] = weights[i] + momentums[i];
                    if (DBG) dbg(`adjust_weights: ${l}:${n} weights[${i}]:${weights[i]} = weights[${i}]:${weights[i]} + momentums[${i}]=${momentums[i]}`);
                }
            }
        }

        if (DBG) dbg(`adjust_weights:- error=${this.error}`);
        return this.error;
    }
}
